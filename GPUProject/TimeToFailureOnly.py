...
test_inputs = total_data[len(total_data) - len(testTimeToFailure) - 50:]
#
#features_set = []  
#labels = []  
#for i in range(50, 500):  
#    features_set.append(trainTimeToFailure[i-50:i, 0])
...
#plt.plot(test_inputs, color='blue', label='Actual Apple Stock Price')  
...
train_data = trainTimeToFailure
test_data = testTimeToFailure
#examine data
np.set_printoptions(threshold=sys.maxsize)
train_data.shape
test_data.shape
...
plt.plot(predictions , color='red', label='Predicted  (batch=32')  
plt.plot(predictions2 , color='green', label='Predicted  (batch=100')  
