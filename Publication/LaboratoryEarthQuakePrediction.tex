...
LANL has since collected data with improved technologies and made it available to the public.\par 

In this {\em new} study we predict the remaining time before laboratory earthquakes occur using the data provided by LANL. We apply machine learning to analyze data patterns, design statistical models and make predictions. We compare predicted versus actual time remaining to determine our accuracy. \par

The results prove that impending laboratory earthquakes can be predicted with 0.5680 Mean Absolute Error {\em null hypothesis, pvalue or confidence interval to be added}. The predictions are based solely on the characteristics of recorded acoustical signals; they are not based on time intervals between prior laboratory earthquakes. The results are an improvement over the prior 2017 LANL experiment.
