...
\institute{Southern Methodist University, Dallas TX 75205, USA \and {\em USGS Fort Worth, TX 76115}}

\maketitle

\begin{abstract}
In 2017 the Los Alamos National Laboratory conducted an experiment which predicted the remaining time before {\em laboratory} earthquakes occured with 90 percent accuracy.\cite{Bertrand}
The technologies used in the laboratory to simulate and collect earthquake data have since improved.\par
Given greater volume and complex data collected with these improved technologies; in this {\em new} study we predict the remaining time before laboratory earthquakes occur. We analyze data patterns using geophysical subject matter expertise, statistical methods and the latest technology available. We design a statistical algorithm to model the patterns and make a prediction. We compare predicted versus actual time remaining to determine our accuracy. \par

Our results prove that our model predicts impending laboratory earthquakes with 0.5680 Mean Absolute Error {\em null hypothesis, pvalue or confidence interval to be added}. The results are an improvement over the 2017 Los Alamos National Laboratory experiment\cite{Bertrand}.
The evidence in this new {\em(observational)} study suggests that {\em We have to find a way to measure against the prior study. We have to state the fact that we cannot prove that the technology improvements are better or that out algorithm is better because of confounding variables such as different collection methods and unavailable details about the 2017 model.} \par

\end{abstract}
\section{INTRODUCTION}
%1 Paragraph Motivtion (Sets General problem domain)
In August 2017 LANL conducted an experiment\cite{Bertrand} which predicted the remaining time until \emph{laboratory} earthquakes occur with 90\% accuracy. The team imitated a real earthquake in the laboratory using steel blocks interacting with rocky material (fault gouge) to induce slipping that emitted seismic acoustic signals. An accelerometer recorded the acoustic signals emanating from the sheared layers. A random forest algorithm was trained with the acoustic signals and stick‐slip frictional failures (actual laboratory earthquakes). It was then used to generate predictions which were measured against actuals.\cite{LANLNews}\par

There have been improvements in the technology used to collect and measure laboratory seismic signal data since the 2017 experiment. The instruments used are ... {\em (additional facts to be gathered)} and LANL is now providing data collected with this improved technology to the public via a competition. \par
\begin{quote}
